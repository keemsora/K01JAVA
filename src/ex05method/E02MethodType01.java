...
Java에서의 Naming rule(이름 규칙)
	- 클래스명: SimpleFunc -> 대문자로 시작하는 Camel case
 	...
 	- 패키지명 : simple.func -> 전체를 소문자로 표현하고, 연결된 단어가 있더라도
 	 						언더바를 사용하지 않는다. 대신 .(닷 혹은 점)을 표현한다.
 */
...
public class E02MethodType01 {
	...
	static void noParamNoReturn2(int a) {
		// 동일한 이름의 함수가 정의되었으나 매개변수의 갯수로 구분 가능하므로 다른 함수임
	}
	...
	static void noParamNoReturn3() {
		...
		//return;
		/*
		위와 값이 함수 중간에 return을 쓰게 되면 함수가 즉시 종료된다.
		즉, 아래 문장은 실행되지 않는 쓰레기코드가 되므로 반드시 조건문과 함께 사용되어야 한다. 
		함수의 마지막 라인에 있는 것은 상관없다.
		 */
